...
const StyleTag = () => (
    ...
    .project-page-container { margin: 2rem auto; padding: 2rem; box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; }
    .page-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem; }
    .page-header h1 { margin: 0; color: var(--text-primary); }
    .add-project-button { padding: 0.6rem 1.2rem; border: none; border-radius: 6px; background-color: var(--button-primary-bg); color: white; font-size: 0.9rem; font-weight: bold; cursor: pointer; transition: background-color 0.2s; }
    .add-project-button:hover { background-color: var(--button-primary-hover); }
    .project-list-container { background-color: var(--content-bg); border-radius: 8px; box-shadow: var(--shadow); overflow: hidden; }
    ...
    .form-group input, .form-group select, .form-group textarea { width: 100%; padding: 0.75rem; border: 1px solid var(--border-color); border-radius: 6px; font-size: 1rem; box-sizing: border-box; }
    ...
    .error-message { color: #e74c3c; text-align: center; padding: 1rem; background-color: #fee2e2; border-radius: 6px; margin: 1rem 0; }
...
const AddProjectModal = ({ onClose, onProjectAdded }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("http://localhost/BugTracker/api/project/create_project.php", {
        method: "POST",
...
const EditProjectModal = ({ project, onClose, onProjectUpdated }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch("http://localhost/BugTracker/api/project/update_project.php", {
        method: "POST", 
        headers: { "Content-Type": "application/json" }, 
        body: JSON.stringify(formData),
      });
...
const ConfirmDeleteModal = ({ project, onClose, onProjectDeleted }) => {
  const [isDeleting, setIsDeleting] = useState(false);

  const handleDelete = async () => {
    setIsDeleting(true);
    try {
      const response = await fetch("http://localhost/BugTracker/api/project/delete_project.php", {
        method: "POST", 
        headers: { "Content-Type": "application/json" }, 
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        <div className="confirm-dialog">
          ...
          <p>Are you sure you want to delete project <strong>{project.name}</strong>? This action cannot be undone.</p>
...
function ProjectManagementPage() {
  ...
  // NEW: Determine if the Actions column should be visible at all
  const canPerformActions = hasPermission('edit_project') || hasPermission('delete_project');

  const fetchProjects = useCallback(async () => {
    setIsLoading(true);
    setError(null);
    try {
      const response = await fetch("http://localhost/BugTracker/api/project/get_projects.php");
      if (!response.ok) {
  ...
  useEffect(() => {
    ...
    refreshPermissions();
    fetchProjects();
  }, [fetchProjects, currentUser, setPermissions]);
  ...
  const handleToggleStatus = async (project) => {
    ...
    try {
      const response = await fetch("http://localhost/BugTracker/api/project/update_project_status.php", {
        method: "POST", 
        headers: { "Content-Type": "application/json" }, 
  ...
  const renderModal = () => {
    ...
    switch (type) {
      case 'add':
        return <AddProjectModal onClose={() => setModal({ type: null, project: null })} onProjectAdded={refreshAndClose} />;
      case 'edit':
        return <EditProjectModal project={project} onClose={() => setModal({ type: null, project: null })} onProjectUpdated={refreshAndClose} />;
      case 'delete':
        return <ConfirmDeleteModal project={project} onClose={() => setModal({ type: null, project: null })} onProjectDeleted={refreshAndClose} />;
      default:
        return null;
    }
  };
  ...
  if (authLoading) {
    return <div className="project-page-container"><div className="spinner"></div></div>;
  }
      ...
      <div className="project-page-container">
        <div className="page-header">
          ...
          {hasPermission('create_project') && (
            <button className="add-project-button" onClick={() => setModal({ type: 'add', project: null })}>
              + New Project
            </button>
          )}
        </div>
            ...
            <table className="custom-table">
              ...
              <tbody>
                {projects.length > 0 ? (
                          ...
                          {hasPermission('edit_project') && (
                            <>
                              <ToggleSwitch 
                                ...
                                handleToggle={() => handleToggleStatus(project)} 
                              />
                              <button className="icon-btn" title="Edit" onClick={() => setModal({ type: 'edit', project })}>
                                <FaPencilAlt />
                              </button>
                            </>
                          )}
                          {hasPermission('delete_project') && (
                            <button className="icon-btn delete" title="Delete" onClick={() => setModal({ type: 'delete', project })}>
                              <FaTrash />
                            </button>
                          )}
                        </td>
                  ...
                  <tr>
                    {/* MODIFIED: Colspan is now dynamic */}
                    <td colSpan={canPerformActions ? 5 : 4} style={{ textAlign: 'center', padding: '1rem' }}>
